...
from utils_AT import calculate_accuracyMax3, calculate_accuracyMaxVot3 # three output
...
#            y_train = y_traint
    ...
    def create_classifier(classifier_name, input_shape, nb_classes, nb_classes_last, output_directory, dataset_name, verbose = False):
        ...
        if classifier_name=='resnet_AT5_base_twoIn6_10':#test iblock original resnet
            from  classifiers import resnet_AT5_base_twoIn6_10 
            return resnet_AT5_base_twoIn6_10.Classifier_RESNET(output_directory,input_shape, nb_classes, nb_classes_last, dataset_name, root_dir, verbose)     
        if classifier_name=='resnet_AT5_base_twoIn6_10_res':#test iblock original resnet
            from  classifiers import resnet_AT5_base_twoIn6_10_res 
            return resnet_AT5_base_twoIn6_10_res.Classifier_RESNET(output_directory,input_shape, nb_classes, nb_classes_last, dataset_name, root_dir, verbose)     
...
##### # make the min to zero of labels        
    ...
    ################################################## remove the lable of out2: y:[o1,o3]
    def o1o3(datasets_dic_a, val=0, down_sample=down_sample):# remove the o2:[o1,o2,o3]=>[o1,o3]
        y_train=datasets_dic_a[dataset_name][1]
        y_train.pop(1) #remove out2:[o1,o3]
        y_val = datasets_dic_a[dataset_name][3]
        y_val.pop(1)
        if val==1:
            y_test = datasets_dic_a[dataset_name][5]
            y_test.pop(1)
        if down_sample==1:
            datasets_dict2  = {dataset_name:(datasets_dic_a[dataset_name][0].copy(), y_train.copy(), datasets_dic_a[dataset_name][2].copy(), y_val.copy(), datasets_dic_a[dataset_name][4].copy(), y_test.copy(), datasets_dic_a[dataset_name][6].copy(), datasets_dic_a[dataset_name][7].copy())}
        else:
            datasets_dict2  = {dataset_name:(datasets_dic_a[dataset_name][0].copy(), y_train.copy(), datasets_dic_a[dataset_name][2].copy(), y_val.copy(), datasets_dic_a[dataset_name][4].copy(), y_test.copy())}
            
        return datasets_dict2
    ###############################
    fit_classifier(w_p)
    ##############################
    end_time = time.time()
