...
#    mts_root_dir = 'C:\Users\ERD204\Documents\data\time_series\archives\mtsdata\' 
    mts_root_dir = 'C:/Users/ERD204/Documents/data/time_series/archives/mtsdata/'#laptop

#    mts_out_dir = '/mnt/nfs/casimir/archives/mts_archive/'
    mts_out_dir = 'C:/Users/ERD204/Documents/data/time_series/archives/mts_archive/'#laptop
...
##########AT:
        for i in range(len(dt)):
            if dt[i] == 'train':
                x_train=a[dt[i]].reshape(max(a[dt[i]].shape))
            elif dt[i] == 'test':
                x_test = a[dt[i]].reshape(max(a[dt[i]].shape))
            elif dt[i]=='trainlabels':
                y_train = a[dt[i]].reshape(max(a[dt[i]].shape))
            elif dt[i]=='testlabels':
                y_test = a[dt[i]].reshape(max(a[dt[i]].shape))    
        # x_train = a[1][0]
        # y_train = a[0][:,0]
        # x_test = a[3][0]
        # y_test = a[2][:,0]
